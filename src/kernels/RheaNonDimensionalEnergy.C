...
RheaNonDimensionalEnergy::RheaNonDimensionalEnergy(const InputParameters & parameters) :
  Kernel(parameters),
    // Coupled values:
...
Real RheaNonDimensionalEnergy::computeQpResidual()
{
  // Convection term:
  ...
  Real relaxation = _sigma_a[_qp]*(temp4-_epsilon[_qp]);
  relaxation *= _SIGMA;
...
Real RheaNonDimensionalEnergy::computeQpJacobian()
{
//  // Convection term:
//  Real vel = _rhou[_qp] / _rho[_qp];
...
Real RheaNonDimensionalEnergy::computeQpOffDiagJacobian( unsigned int _jvar)
{
//  if (_jvar == _rho_nb)
//  {
//    // Convection term:
