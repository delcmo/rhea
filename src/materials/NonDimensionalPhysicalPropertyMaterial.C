...
NonDimensionalPhysicalPropertyMaterial::NonDimensionalPhysicalPropertyMaterial(const InputParameters & params) :
    Material(params),
    ...
    _ics(getUserObject<ComputeICsRadHydro>("ics"))
{
  ...
  if (_cs_type == temp_dpt_cs)
  {
    ...
    _sigma_hat_t = _sigma_t0(0);
    _sigma_hat_a = _sigma_a0(0);
  }
...
void
NonDimensionalPhysicalPropertyMaterial::computeQpProperties()
{
  // Cross sections:
  Real temp = 0.;
  switch (_cs_type)
